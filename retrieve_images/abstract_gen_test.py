...
from prompt_purfirst import ABSTRACT_PROMPT_PURPOSE, ABSTRACT_PROMPT, SYSTEM_PROMPT
...
def generate_description(images: list[str], prompt: str, system: str):
    ...
    generated_ids = model.generate(**inputs, max_new_tokens=4096, temperature=1e-5)
...
def writing_file(args, output_list):
...
def batch_processing(args, images_dir_list):
    ...
    for i in range(0, len(images_dir_list), args.batch_size):
        ...
        dict_output = generate_description(images_path, ABSTRACT_PROMPT_PURPOSE(), SYSTEM_PROMPT)
...
def last_answer(args, images_dir_list):
    # Process image in batch
    text_output = []
    ...
    for i in range(0, len(images_dir_list), args.batch_size):
        images_path = images_dir_list[i: i + args.batch_size]
        batch_output = [{} for _ in range(args.batch_size)]
        for j in range(0, 7, args.num_topic):
            dict_output = generate_description(images_path, ABSTRACT_PROMPT(range(j, min(j + args.num_topic, 7))), SYSTEM_PROMPT)
            for k in range(args.batch_size):
                # print(dict_output[k].strip("```json"))
                dict_output[k] = json.loads(dict_output[k].strip("```json"))
                batch_output[k] |= dict_output[k]
...
def main():
    ...
    parser.add_argument("--purpose_only", type=bool, default=False)
    ...
    output_list = batch_processing(args, images_dir_list) # Always use 1 for 7B model due to context limits

    if not args.purpose_only:
        output_list = last_answer(args, images_dir_list)

    open(args.output_file, "w").close()
    writing_file(args, output_list)
