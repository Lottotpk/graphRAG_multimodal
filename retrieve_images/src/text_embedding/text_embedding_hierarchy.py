...
The database will store summary, entities, relations, abstracts separately instead of combining it together like
text_embedding.py
"""
...
def main(args):
    ...
    for it in items:
        if not it.get('error'):
            for component in ("summary", "entities", "relations"):
                texts.append(it[component])
                meta.append({'source_image_path': it.get('image_path'), 'type': component})
            texts.append(combine_abstract(it["abstract"]))
            meta.append({'source_image_path': it.get('image_path'), 'type': 'abstract'})

    if not texts:
        logger.info('No valid descriptions found in JSON')
        sys.exit(0)

    logger.info(f'Found {len(texts) // 4} (*4) descriptions. Loading model...')
