...
def combine_abstract(desc: dict):
    abstract = ""
        ...
        while True:
            ...
            try:
                ...
                if loop == 3:
                    ...
                    continue
            try:
                prompt['format_prompt'] = [summary['summary'], summary['entities'], summary['relations'], combine_abstract(abstract)]
                break
...
def main(args: argparse.Namespace):
    ...
    weights = [w / sum(args.weight) for w in args.weight]
    logger.info(f"Running with (normalized) weights [summary, entities, relations, abstract] = {weights}")
    ...
    k = 20
    batch_size = 16 // TOTAL_COMPONENTS
    embeddings = []
    start_time = time.time()
    for i in range(0, total, batch_size):
        batch_texts = prompt[i:min(i+batch_size, len(prompt))]
        batch_texts = [item for texts in batch_texts for item in texts] # Flatten nested query
        embedding, _ = embedder.embed(batch_texts, args.strategy)
        embedding = [x.float().numpy() for x in embedding]
        embedding = [embedding[i:i+TOTAL_COMPONENTS] for i in range(0, len(embedding), TOTAL_COMPONENTS)] # Nest the flat embedding
    ...
    for i in range(total):
        ...
        scores, img_path_retrieved = db.search_hierarchy(embeddings[i], weights, k=k)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weight", type=float, nargs=4, default=[1, 1, 1, 1], metavar=('Summary', 'Entities', 'Relations', 'Abstract'), help='Specify weights')
