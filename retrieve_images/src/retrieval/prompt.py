...
def ABSTRACT_PROMPT(cate_num: range, summary: str) -> str:
    cate_prompt = ""
    example = "{\n"
    output = "{\n"
    ...
    - Query text: The text input to analyze
    ...
    - Explore how keywords and its reason correlate to the Starry Night
    1) Analyze the Query text
    - Identify up to 5 relevant keywords according to the category based on the evidence from the Query text, such as the main characters' expression, the place, and the Query text vibe.
    ...
    - Analyzing grounding behind the reasons: whether each reason from keyword can be linked to the visual evidence in the Query text.
    ...
    - Mentally simulate the situation using the "Purpose/Context" category in the image from this text whether the keyword intuitively make sense in that specific context or not.
    4) Summarize & Score
    - Write keywords and its reason after evaluation
    - Output a confidence score (0.0–1.0), where 1.0 = highly relevant, 0.0 = irrelevant reflecting: the relevance between keyword and reason and Query text, the mental simulation, and the grounding candidates observed from the Query text.
    5) Selection
    ...
    - Category with more than 3 keywords, choose only the keywords with top 3 highest confidence number.
    ...
    - Avoid generic keyword
    - At most 3 keywords per "category", no more than this; can use less than this
    - Reasons and evidence can be found from the Query text; No making up the answer.
    - Using the example only as a reference, generate the output for the Query text.
    - All the keywords with confidence score lower than 0.7 are excluded from the output.
""" % (summary, example, cate_prompt, output)
...
SYSTEM_PROMPT = """
    You are a skilled data extraction agent. 
