#!/usr/bin/env python3
"""
Image Summary Generator (uses local InternVL3_5-8B chat API)
...
def generate_summary(args):
    ensure_image_description_dir("only_summary")
    ...
    filename = get_description_filename(prompt_slug='desc')
    out_path = get_description_path(filename, "only_summary")
    payload = {
        'created_at': datetime.now().isoformat(),
        'system_prompt': SYSTEM_PROMPT,
        'prompt': PROMPT,
