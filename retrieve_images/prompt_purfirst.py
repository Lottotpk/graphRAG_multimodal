...
TARGET_AUDIENCE_PERCEPTION = """
    ...
    - [FIELD] Expert (specify FIELD): Positioned to be perceived as credible, trustworthy, and knowledgeable on that specific FIELD, often used in technical or professional contexts.
...
def ABSTRACT_PROMPT_PURPOSE():
    ...
    - Identify up to 3 relevant keywords based on the evidence from the Query image, such as the main characters' expression, the place, and the Query image vibe.
    ...
    - Analyze grounding behind the reasons: whether each reason from keyword can be linked to the visual evidence in the Query image.
    - Verify the keywords: the keyword is the purpose/context to use the image, NOT the action of the main character in the image
    - No making up explanation or implication
    - Mentally simulate the situation using the keyword whether it intuitively makes sense in that specific context to the Query image or not.
    4) Summarize & Score
    ...
    5) Selection
    - Strictly remove all keywords with confidence score lower than 0.7 even though it remains zero keywords.
    - Zero keywords can simply use [] (the empty list in response)
    ...
    {
        "Purpose/Context" : [{"keyword": "", "reason": "", "confidence": <number between 0.0 and 1.0>}]
    }
    ...
    - At most 3 keywords, no more than this; can use less than this
...
CATEGORY = [EMOTION_MOOD, STYLE_VISUAL_ATTR, MEDIUM_MATERIAL, COLOR_LIGHTNING_CHAR, CULTURAL_REGIONAL, TARGET_AUDIENCE_PERCEPTION, NARRATIVE_SYMBOLIC]
CATEGORY_NAME = ["Emotion/Mood", "Style/Visual Attributes", "Medium/Material", "Color/Lightning Characteristics", "Cultural/Regional Elements", "Target Audience/Perception", "Narrative/Symbolic Elements"]

def ABSTRACT_PROMPT(cate_num: range, pur_txt: str):
    cate_prompt = ""
    example = "{\n"
    output = "{\n"
    for i, num in enumerate(cate_num):
        ...
        for key in cate_exam[1]:
            example += "\t" + str(key) + ',\n'
        example += "\t],\n"
    example += "\t\"Purpose/Context\": [\n"
    for key in EXAMPLE_IMAGE_ANSWER["Purpose/Context"]:
        example += "\t" + str(key) + ",\n"
    example += "\t],\n}"
    return """
    Inputs (always provided)
    ...
    0) Understand the Example and the given purpose
